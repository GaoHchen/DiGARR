...
import utils.camera_barf as camera_barf
...
@torch.no_grad()
def vis_cameras(
    vis, step, poses=[], colors=["blue", "magenta"], plot_dist=True
):
    win_name = "{}/{}".format("test", "debug")  # todo
    data = []
    # set up plots
    centers = []
...
def get_camera_mesh(pose, depth=1):
    ...
    vertices = camera_barf.cam2world(vertices[None], pose)
...
def plot_save_poses(fig, pose, pose_ref=None, path=None, ep=None):
    # get the camera meshes
    _, _, cam = get_camera_mesh(pose, depth=camera_depth)    # todo
    cam = cam.numpy()
    if pose_ref is not None:
        _, _, cam_ref = get_camera_mesh(pose_ref, depth=camera_depth)
...
def plot_save_poses_blender( fig, pose, pose_ref=None, path=None, ep=None):
    # get the camera meshes
    _, _, cam = get_camera_mesh(pose, depth=camera_depth)
    cam = cam.numpy()
    if pose_ref is not None:
        _, _, cam_ref = get_camera_mesh(pose_ref, depth=camera_depth)
        cam_ref = cam_ref.numpy()
    ...
    # setup_3D_plot(
    #     ax, elev=45, azim=35, lim=edict(x=(-3, -1), y=(-2, 0), z=(-2.2, -1))
    # )
    plt.subplots_adjust(
        left=0, right=1, bottom=0, top=0.95, wspace=0, hspace=0
    )
