...
def init_grid_param(
    ...
    coo_combs = list(itertools.combinations(range(in_dim), grid_nd))  # @remind xy,xz,(xt,)yz,(yt,zt)
...
def interpolate_ms_features(pts: torch.Tensor,
    ...
    for scale_id, grid in enumerate(ms_grids[:num_levels]):
        interp_space = 1.
        # print(grid[0].shape, grid[1].shape,cur_level)
        for ci, coo_comb in enumerate(coo_combs):
            ...
            interp_space = interp_space * interp_out_plane  #FIXME:
        ...
        if scale_id > cur_level:
            ...
            if concat_features:
                multi_scale_interp.append(interp_space)  #@todo 24/01/26: code3x
            else:
...
class KPlaneField(nn.Module):
    ...
    def precompute_code(self, pts: torch.Tensor, timestamps: Optional[torch.Tensor] = None, cur_level=3):
        """precompute point features."""
        if self.spatial_distortion is not None:
            pts = self.spatial_distortion(pts)
            pts = pts / 2  # from [-2, 2] to [-1, 1]
        else:
        ...
        # breakpoint() 
        features = interpolate_ms_features(
            pts=pts, ms_grids=self.grids,  # noqa
            grid_dimensions=self.grid_config[0]["grid_dimensions"],
            concat_features=self.concat_features, num_levels=None, pts_spacetime_detach=self.pts_spacetime_detach,
            cur_level=cur_level)
        return features
    ...
    def init_with_code(self, code: Optional[torch.Tensor]=None):
        # code: l(k(1,ch,*res))
        code_level = len(code)
        if code_level < len(self.grids):
            self.init_with_code_split_channels(code[0])
            return

        for level, grid in enumerate(self.grids):
            c_level = min(level, code_level)
            for ci, grid_coef in enumerate(grid):
                if code is not None:
                    h,w = grid_coef.shape[-2:]
                    self.grids[level][ci].data = F.interpolate(code[c_level][ci].data, size=(h,w), mode='bilinear', align_corners=True)
                else:
                    nn.init.uniform_(self.grids[level][ci], a=0.1, b=0.5)

    def init_with_code_split_channels(self, code: Optional[torch.Tensor]=None):
        # code: (k,1,ch,*res)
        for level, grid in enumerate(self.grids):
            for ci, grid_coef in enumerate(grid):
                if code is not None:
                    h,w = grid_coef.shape[-2:]
                    self.grids[level][ci].data = F.interpolate(code[ci, :, level * self.base_channels : (level+1) * self.base_channels].data, size=(h,w), mode='bilinear', align_corners=True)
                else:
                    nn.init.uniform_(self.grids[level][ci], a=0.1, b=0.5)
    ...
    def forward(self,
        ...
        if self.use_appearance_embedding:
            ...
            timestamps = None
        if global_step is not None and self.scale_step is not None:
            if global_step in self.scale_step:
                log.info(f"Grids scale up to level {self.cur_level+1}")
                self.upsample_grid()
                # log.info(f"pts grad to pose max level up!! From level0 to level{self.cur_level+1}")
                self.cur_level = self.cur_level + 1
