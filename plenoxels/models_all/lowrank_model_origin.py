...
from plenoxels.models_all.density_fields_origin import KPlaneDensityField
from plenoxels.models_all.kplane_field import KPlaneField  #FIXME:
...
class LowrankModel_kp(nn.Module):
    ...
    def init_grids_with_code(self, code: Optional[torch.Tensor], code_proposal: Optional[torch.Tensor]):
        ...
        for i, pn in enumerate(self.proposal_networks):
            pn.init_with_code(code_proposal[i])

    def forward(self, bg_color, near_far: torch.Tensor, rays_o=None, rays_d=None, timestamps=None, global_step=None):  #@note
        """
        rays_o : [batch, 3]
        rays_d : [batch, 3]
        timestamps : [batch]
    ...
    def get_params_ms(self, lr: float, field_lr: list=None):
        model_params = self.field.get_params()
        pn_params = [pn.get_params() for pn in self.proposal_networks]  # density_field
        field_params = model_params["field"] + [p for pnp in pn_params for p in pnp["field"]]
        nn_params = model_params["nn"] + [p for pnp in pn_params for p in pnp["nn"]]
        ...
        if field_lr is None:
            ...
            return params_ms + [{"params": field_params[len(field_lr)*3:], "lr": lr},{"params": nn_params, "lr": lr}, {"params": other_params, "lr": lr}]
