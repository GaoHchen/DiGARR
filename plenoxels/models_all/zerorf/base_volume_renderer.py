...
class VolumeRenderer(nn.Module):
    ...
    def forward(self, rays_o, rays_d, code, density_bitfield, grid_size,
        ...
        code = self.preproc(code)  # lu:(num_scenes, ch_out* sum(noise_res)), 过了encoder #TODO: 这里的梯度传给code了吗？
        ...
        if self.unbounded:
            ...
            if self.occlusion_culling_th > 0:
                with torch.no_grad():
                    sigmas, rgbs, num_points = self.point_decode(xyzs, None, code)  # rgbs=None
                rgbs = sigmas.new_zeros(sigmas.shape[0], 3)
                with torch.no_grad():
                    if sigmas.dtype != dtype or rgbs.dtype != dtype:
                        sigmas, rgbs = sigmas.to(dtype), rgbs.to(dtype)
    ...
    def point_decode(self, xyzs, dirs, code, density_only=False):
        ...
        if isinstance(xyzs, torch.Tensor):
            ...
            for i, xyzs_single in enumerate(xyzs):
                ...
                point_code_single = self.get_point_code(code[i: i + 1], xyzs_single[None])  # (num_points_per_scene, self.in_chs)
                num_points.append(num_points_per_scene)
                point_code.append(point_code_single)
            point_code = torch.cat(point_code, dim=0) if len(point_code) > 1 else point_code[0]  # (num_scenes*num_points_per_scene, self.in_chs)
