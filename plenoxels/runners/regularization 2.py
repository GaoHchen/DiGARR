...
class PlaneTV_kp(Regularizer):
    ...
    def _regularize(self, model: LowrankModel, **kwargs):
        ...
        if self.what == 'field':
            ...
            multi_res_grids = [p.grids for p in model.proposal_networks_getcode]
        else:
...
class PlaneTV(Regularizer):
    ...
    def _regularize(self, model: LowrankModel, **kwargs):
        if self.what == 'field':
            # codes = [model.code]
            codes = model.code  # l(k(1,ch,*res))
            # codes = model.code_conv
        elif self.what == 'proposal_network':
            codes = model.code_proposal
        else:
            raise NotImplementedError(self.what)
        total = 0
        # Note: input to compute_plane_tv should be of shape [batch_size, c, h, w]
        for code in codes:
            spatial_grids = [0, 1, 2]  # These are the spatial grids; the others are spatiotemporal
            for grid_id in spatial_grids:
                total += compute_plane_tv(code[grid_id])
            for grid in code:
                # grid: [1, c, h, w]
                total += compute_plane_tv(grid)
        return total
...
class TimeSmoothness_kp(Regularizer):
    def __init__(self, initial_value, what: str = 'field'):
        if what not in {'field', 'proposal_network'}:
            raise ValueError(f'what must be one of "field" or "proposal_network" '
                             f'but {what} was passed.')
...
class TimeSmoothness(Regularizer):
    def __init__(self, initial_value, what='field'):
        if what not in {'field', 'proposal_network'}:
            raise ValueError(f'what must be one of "field" or "proposal_network" '
                             f'but {what} was passed.')
        super().__init__(f'time-smooth-{what[:2]}', initial_value)
        self.what = what

    def _regularize(self, model: LowrankModel, **kwargs) -> torch.Tensor:
        # model.code.shape is k(1, ch, *res)
        if self.what == 'field':
            codes = model.code
        elif self.what == 'proposal_network':
            codes = model.code_proposal  # list(codelist)
        else:
            raise NotImplementedError(self.what)
        total = 0.0
        spatiotemporal_grids = [3, 4, 5]
        # for i,cfg in enumerate(model.preprocessor.tensor_config):
        #     if 't' in cfg:
        #         total = total + torch.abs(1 - model.code[i]).mean()
        for code in codes:
            for plane_idx in spatiotemporal_grids:
                total = total + compute_plane_smoothness(code[plane_idx]).mean()
        return torch.as_tensor(total)
...
class L1TimePlanes_kp(Regularizer):
    def __init__(self, initial_value, what='field'):
        if what not in {'field', 'proposal_network'}:
            raise ValueError(f'what must be one of "field" or "proposal_network" '
                             f'but {what} was passed.')
...
class L1TimePlanes(Regularizer):
    ...
    def _regularize(self, model: LowrankModel, **kwargs) -> torch.Tensor:
        ...
        for code in codes:
            for plane_idx in spatiotemporal_grids:
                total = total + torch.abs(1 - code[plane_idx]).mean()
        return torch.as_tensor(total)
