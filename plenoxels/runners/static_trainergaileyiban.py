...
class StaticTrainer(BaseTrainer):
    ...
    def eval_step(self, data, **kwargs) -> MutableMapping[str, torch.Tensor]:  #@todo
        """
        Note that here `data` contains a whole image. we need to split it up before tracing
        for memory constraints.
        """
        ...
        with torch.cuda.amp.autocast(enabled=self.train_fp16), torch.no_grad():
            ...
            # near_far, camera_id, timestamps, intrinsics and bg_color are constant over mini-batches
            near_far = data["near_fars"].to(self.device)
            ...
            if isinstance(bg_color, torch.Tensor):
                bg_color = bg_color.to(self.device)
            preds = defaultdict(list)
            for b in range(math.ceil(camera_dirs.shape[0] / batch_size)):  # camera_dirs.shape[0]:h*w
                camera_dirs_b = camera_dirs[b * batch_size: (b + 1) * batch_size].to(self.device)
                outputs = self.model(rays_o_b, rays_d_b, near_far=near_far,
                                     bg_color=bg_color)
                for k, v in outputs.items():
                    if k in channels or "depth" in k:
