...
class LowrankModel(nn.Module):
    def __init__(self,
                 ...
                 # posenet settings
                 num_cams: Optional[int] = None,  # 从base_trainer传过来的
                 learn_R: bool = False,
                 learn_t: bool = False,
                 **kwargs,
                 ):
    ...
    def forward(self, intrinsics, camera_dirs, camera_id, bg_color, near_far: torch.Tensor, timestamps=None, process_tsdata=False, bs_size=None):  #@todo get pose -> get rays_o, rays_d
        """
        # rays_o : [batch, 3]
        # rays_d : [batch, 3]
        intrinsics : [3, 3]
        ...
        if self.pose_param_net is not None:
            ...
            c2w = self.pose_param_net(camera_id)  # [n_rays, 3, 4]
    ...
    def get_params(self, lr: float, pose_lr: float):
        model_params = self.field.get_params()
        pose_params = self.pose_param_net.get_params()["pose"]
