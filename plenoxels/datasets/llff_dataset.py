...
from .feature_extractor import DinoWrapper
...
class LLFFDataset(BaseDataset):
    def __init__(self,
        ...
        image_paths = sorted(glob.glob(os.path.join(datadir, f'images_4/*')))
        self.extractor = DinoWrapper(image_paths[0])    # TODO: choose which image to extract feature? or choose all?
        self.first_img_feature = self.extractor()   # Batch, Num of token, Channels
        ...
        if split == 'render':
            ...
            image_paths, poses, near_fars, intrinsics, num_cams_train = load_llff_poses(
            ...
            image_paths, dino_paths, self.poses, near_fars, intrinsics, num_cams_train = load_llff_poses(
        ...
        # if split == "train":
        #     dino_feats = []
        #     for img_path in dino_paths:
        #         extractor = DinoWrapper(img_path)
        #         dino_feats.append(extractor())
        #     self.dino_feats = torch.cat(dino_feats, dim=0).to(imgs.device)
        ...
        if contraction:
            ...
            self.near_fars = torch.tensor([[0.0, self.ndc_far]]).repeat(self.num_images, 1)
    ...
    def __getitem__(self, index):
        ...
        #pixtocam = torch.linalg.inv(self.intrinsics.get_matrix())
        #out["pixtocam"] = pixtocam
        out["intrinsics"] = self.intrinsics.get_matrix()

        # if self.split == "train" and self.dino_feats is not None:
        #     out["dino_feat"] = self.dino_feats[image_id]
...
def load_llff_poses(datadir: str,
    ...
    if int_dsample != downsample or int_dsample not in {1, 4, 8}:
    ...
    # if int_dsample == 1:
    #     image_paths = sorted(glob.glob(os.path.join(datadir, f'images/*')))
    # else:
    #     image_paths = sorted(glob.glob(os.path.join(datadir, f'images_{int_dsample}/*')))
    image_paths = sorted(glob.glob(os.path.join(datadir, f'images/*')))
    dino_paths = sorted(glob.glob(os.path.join(datadir, f'images_8/*')))
    # breakpoint()
    ...
    # i_test = np.array([4,8,12])
    # i_test = np.array([1,9,17])
    i_test = np.arange(1, poses.shape[0], hold_every)
    # keep last 10% as test
    # breakpoint()
    # i_test = np.arange(poses.shape[0] - int(0.1 * poses.shape[0]), poses.shape[0], 1) # TODO:
    ...
    dino_paths = [dino_paths[i] for i in img_list]
    poses = torch.from_numpy(poses[img_list]).float()
    near_fars = torch.from_numpy(near_fars[img_list]).float()

    if split == 'train_render':
        intrinsics.scale(1/4)  # 1/12 @remind load pose 的时候已经将 intrinsics downscale 了 downsample 倍. if downsample=4, then imgshape (756,1008) --> (63,84)

    # breakpoint()
    poses[:,0,:] = poses[:,0,:] * -1  #@remind

    return image_paths, dino_paths, poses, near_fars, intrinsics, num_cams_train
