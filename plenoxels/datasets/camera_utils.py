...
def pixels_to_rays(pix_x_int, pix_y_int, pixtocams,
    ...
    """Calculates rays given pixel coordinates, intrinisics, and extrinsics. @note get radii
  ...
  """
    ...
    # Must add half pixel offset to shoot rays through pixel centers.  @todo 为什么? A: 使射线穿过像素中心（减少真实位置误差）
    def pix_to_dir(x, y):
    ...
    pixel_dirs_stacked = np.stack([
        ...
        pix_to_dir(pix_x_int + 1, pix_y_int),  # lu: for dx
        pix_to_dir(pix_x_int, pix_y_int + 1)  # lu: for dy
    ], axis=0)
    ...
    if distortion_params is not None:
        ...
        x, y = _radial_and_tangential_undistort(
            camera_dirs_stacked[..., 0],  # lu: x
            camera_dirs_stacked[..., 1],  # lu: y
            **distortion_params)
        camera_dirs_stacked = np.stack([x, y, np.ones_like(x)], -1)

    if camtype == ProjectionType.FISHEYE:  # lu: 鱼眼
    ...
    imageplane = camera_dirs_stacked[0, ..., :2]  # lu: cam coord

    # Apply camera rotation matrices.
    directions_stacked = mat_vec_mul(camtoworlds[..., :3, :3],
                                     camera_dirs_stacked)  # lu: world coord
    # Extract the offset rays.
    directions, dx, dy = directions_stacked  # lu: directions是 world coord pixel_center(plus half了)

    origins = np.broadcast_to(camtoworlds[..., :3, -1], directions.shape)  # lu: cam center(world coord)
    viewdirs = directions / np.linalg.norm(directions, axis=-1, keepdims=True)  # lu: 长度为1的方向

    if pixtocam_ndc is None:  # lu: not ndc
        ...
        # In NDC space, we use the offset between origins instead of directions. @todo why?
