...
def _load_video_1cam(idx: int,
    ...
    for frame_idx, frame in enumerate(all_frames):
        if frame_idx % load_every != 0:
            continue
        if frame_idx >= n_frame_per_cam:#300:  # Only look at the first 10 seconds  @todo
            break
    ...
    imgs = torch.stack(imgs, 0)  # [n_frames_per_cam=300, h, w, 3]
    med_img, _ = torch.median(imgs, dim=0)  # [h, w, 3]
    return (imgs,
            poses[idx].expand(len(timestamps), -1, -1),
            med_img,
            torch.tensor(timestamps, dtype=torch.int32))  # [n_frames_per_cam=300]
