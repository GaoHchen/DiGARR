...
class PDFSampler(Sampler):
    ...
    def generate_ray_samples(
        ...
        inds = torch.searchsorted(cdf, u, right=True)  # 在给定的排序序列cdf中搜索值u所对应的索引
        ...
        if self.include_original:  # False
...
class ProposalNetworkSampler(Sampler):
    ...
    def generate_ray_samples(
        ...
        preproc: Optional[List[Callable]] = None,
        density_fns: Optional[List[Callable]] = None,
    ) -> Tuple[RaySamples, List, List]:
        assert ray_bundle is not None
        # assert preproc is not None
        assert density_fns is not None
        # assert len(preproc) == self.num_proposal_network_iterations
        ...
        for i_level in range(n + 1):  # initial_sample, n*proposal_sample
            is_prop = i_level < n
            num_samples = self.num_proposal_samples_per_ray[i_level] if is_prop else self.num_nerf_samples_per_ray  # [256,128,48]
            if i_level == 0:
            ...
            if is_prop:  # 不是最後一次sample
                if updated:
                    ...
                    if preproc is None:
                        ...
                        code = preproc[i_level]()
                        code_list.append(code)
                        code = torch.cat([torch.flatten(c,1) for c in code], 1)  # (1, k * ch * res**2)
                        assert not torch.isnan(code).any(), (f"Codes become nan!!!!!\n{code.view(3,-1)}")
                        density = density_fns[i_level](ray_samples.get_positions().detach(), timestamps, code)  #@todo detach() or not
                else:
                    with torch.no_grad():
                        if preproc is None:
                            density = density_fns[i_level](ray_samples.get_positions(), timestamps)  # world space
                            ...
                            code_list.append(code)
                            code = torch.cat([torch.flatten(c,1) for c in code], 1)  # (1, k * ch * res**2)
                            density = density_fns[i_level](ray_samples.get_positions().detach(), timestamps, code)
        ...
        if preproc is None:
            ...
            return ray_samples, weights_list, ray_samples_list, code_list
        
        # 1/12
    def generate_ray_samples_precompute_codes(
        self,
        ray_bundle: Optional[RayBundle] = None,
        timestamps: Optional[float] = None,
        # code: Optional[torch.Tensor] = None,
        preproc: Optional[List[Callable]] = None,
        density_fns: Optional[List[Callable]] = None,
        precompute_fns : Optional[List[torch.Tensor]] = None,
    ) -> Tuple[RaySamples, List, List]:
        assert ray_bundle is not None
        # assert preproc is not None
        assert density_fns is not None
        ...
        for i_level in range(n + 1):  # initial_sample, n*proposal_sample
            ...
            if is_prop:  # 不是最後一次sample
                if updated:
                    # always update on the first step or the inf check in grad scaling crashes
                    # if preproc is None:
                    # 1/12 generate precompute codes, use kplanes ms_interpolations, the use 1 stage decoder
                    code_precomp = precompute_fns[i_level](ray_samples.get_positions(), timestamps)
                    density = density_fns[i_level](ray_samples.get_positions().detach(), timestamps, code_precomp)  # world space detach? why
                    # else:
                    #     # import ipdb; ipdb.set_trace()
                    #     code = preproc[i_level]()
                    #     code_list.append(code)
                    ...
                    with torch.no_grad():
                        # if preproc is None:
                        code_precomp = precompute_fns[i_level](ray_samples.get_positions(), timestamps)
                        density = density_fns[i_level](ray_samples.get_positions().detach(), timestamps, code_precomp)  # world space
                        # else:
                        #     code = preproc[i_level]()
                        #     code_list.append(code)
                        #     code = torch.cat([torch.flatten(c,1) for c in code], 1)  # (1, k * ch * res**2)
